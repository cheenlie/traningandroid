...
import com.pmkebiao.timetable.ToolsUtil;
...
//	View saveView;
	...
	public TableMainActivity() {
		...
		year_c = Integer.parseInt(currentTimeString.split("-")[0]);
		month_c = Integer.parseInt(currentTimeString.split("-")[1]);
		day_c = Integer.parseInt(currentTimeString.split("-")[2]);
		currentYear = year_c;
		currentMonth = month_c;
		specialCalendar = new SpecialCalendar();
		getCalendar(year_c, month_c);
		week_num = MyTimeUtil.getWeeksOfMonth(dayOfWeek,daysOfMonth,true);
		currentNum = week_num;
...
//		relativelayout_main_contain_flipper_timelist = (RelativeLayout) findViewById(R.id.relativelayout_main_all_contain_flipper_timelist);
		...
		widPx = (width - ToolsUtil.dipTopx(TableMainActivity.this, 25)) / 7;
...
//		weeks_nowBetween20140922 = MyTimeUtil.getWeeks(currentDate);
	...
	private void updateGridlayout(int weekOfClassBegin, GridLayout tableGridLayout) {
		...
		heightTotal = (ToolsUtil.dipTopx(TableMainActivity.this, 56));
		scrollHeightOnePX = (ToolsUtil.dipTopx(TableMainActivity.this, 60));
		weekAdd = weekOfClassBegin;
		height = (ToolsUtil.dipTopx(TableMainActivity.this, 60)) / 6 - 4;
...
//	
	...
	public void getCurrent() {
		if (currentWeek > currentNum) {
			...
			currentWeek = 1;
			currentNum = MyTimeUtil.getWeeksOfMonth(currentYear, currentMonth);
